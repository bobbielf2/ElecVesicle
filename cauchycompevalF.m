...
if nargout==1  % no deriv wanted... (note sum along 1-axis faster than 2-axis)  
  ...
  % Replaced - Remember transpose
  I02=comp.'*vb;
  %I0 = sum(repmat(vb(:),[1 M]).*comp); 
  
  J0 = sum(pcomp); % Ioakimidis notation
  
  % Repalced - Remember transpose
  vc2=diag(1./J0)*I02;
  %vc = I0./J0;                       % bary form
  
    % Replaced - Remember transpose
  if side=='e'
      vc2=diag(1./(x(:).'-s.a))*vc2;
      %vc = vc./(x(:).'-s.a); 
  end         % correct w/ pole
  
  [jj,ii] = ind2sub(size(comp),find(~isfinite(comp))); % node-targ coincidences

    % Replaced - could not confirm
  for l=1:numel(jj)
      vc2(ii(l),:) = vb(jj(l),:);
     % vc(ii(l)) = vb(jj(l));
  end % replace each hit w/ corresp vb
    vc=vc2.';
...
%   end
  ...
  % Replaced - Remember transpose - This does not include bary corrections
  vcp2=((prefac./J0).'*ones(1,N)).*((comp.*invd).'*vb-((ones(1,N)*(comp.*invd)).'*ones(1,N)).*vc2);
  ...
  for l=1:numel(jj), j=jj(l); i=ii(l); % loop over hitting node-targ pairs
    ...
    % Replaced - This has not been checked
    vc2(i,:)=vb(j,:);
    ...
    % Replaced - This had not been checked
    vcp2(i,:)=(cw(notj)./(s.x(j)-s.x(notj))/cw(j)).'*(ones(N-1,1)*vb(j,:)-vb(notj,:));
