...
if side=='e'
  ...
  vb2=vb2-ones(length(s.x),1)*vinf2;
...
if wantder,
    ...
    [v2,vp2] = cauchycompevalF(x,s,vb2,side);
    if side=='e'
        vp2=vp2+1./(s.a - x)*totchgp2;
        %vp = vp + totchgp./(s.a - x(:).');
    end % add log deriv
    ux = real(vp2);
    uy = -imag(vp2);
    ...
    v2 = cauchycompevalF(x,s,vb2,side);
end
%u=real(v2)*tau;
%u = real(v)';
if side=='e'         % add real part of log and of v_infty back in...
    
    u = real(v2)- log(abs(s.a - x(:)))*totchgp2 + ones(length(x),1)*real(vinf2);
